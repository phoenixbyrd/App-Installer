...
declare -A APP_CONFIG=(
    [url]="https://github.com/Pi-Apps-Coders/files/releases/download/large-files/prismlauncher_9.0-1_arm64.deb"
    [name]="prismlauncher"
    ...
    [category]="Game;"
    [dependencies]="mesa-utils libopenal1 openjdk-17-jre"
...
install_app() {
    ...
    $prun sudo dpkg -i "./$deb_name" || {
        ...
        $prun sudo dpkg -i "./$deb_name" || error "Failed to install .deb package"
    }

    # Try to find the binary name (usually matches package name)
    local binary_name="${APP_CONFIG[name]}"
